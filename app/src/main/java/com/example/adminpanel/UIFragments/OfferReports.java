...
public class OfferReports extends Fragment {
    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        return inflater.inflate(R.layout.post_reports_fragment, container, false);
    }
    ...
    private void offerReportApiCall() {
        try {
            ...
            StringRequest request = new StringRequest(
                    Request.Method.GET,
                    ApiCollection.getReportedOffers + LoginService.getAdminId(),
                    response -> {
                        refreshLayout.setRefreshing(false);
                        loading.setVisibility(View.GONE);
                        recyclerView.setVisibility(View.VISIBLE);
                        try {
                            ReportedOfferResponse offerResponse = new GsonBuilder().create().fromJson(
                                    response, ReportedOfferResponse.class
                            );
                            if (offerResponse.getSuccess()) {
                                if (OfferReportsService.addAll(offerResponse.getResult())) {
                                    fillUpRecycler();
                                }
                            }
                        } catch (Exception ex) {
    ...
    private void fillUpRecycler() {
        try {
            ...
            /*TODO: make adapter*/
            ReportedOfferAdapter offerAdapter = new ReportedOfferAdapter(getContext(), OfferReportsService.offersList);
